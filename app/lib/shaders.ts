/** 채도·대비·색조 uniform을 셰이더에 주입하는 공통 코드 */

export const COLOR_COMMON_GLSL = `
#include <common>
uniform float saturation;
uniform float contrast;
`;

export const COLOR_COMMON_WITH_HUE_GLSL = `
#include <common>
uniform float saturation;
uniform float contrast;
uniform float hue;

vec3 rgb2hsv(vec3 c) {
  vec4 K = vec4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
  vec4 p = mix(vec4(c.bg, K.wz), vec4(c.gb, K.xy), step(c.b, c.g));
  vec4 q = mix(vec4(p.xyw, c.r), vec4(c.r, p.yzx), step(p.x, c.r));
  float d = q.x - min(q.w, q.y);
  float e = 1.0e-10;
  return vec3(abs(q.z + (q.w - q.y) / (6.0 * d + e)), d / (q.x + e), q.x);
}

vec3 hsv2rgb(vec3 c) {
  vec4 K = vec4(1.0, 2.0 / 3.0, 1.0 / 3.0, 3.0);
  vec3 p = abs(fract(c.xxx + K.xyz) * 6.0 - K.www);
  return c.z * mix(K.xxx, clamp(p - K.xxx, 0.0, 1.0), c.y);
}
`;

export const COLOR_FRAGMENT_GLSL = `
#include <color_fragment>
diffuseColor.rgb = (diffuseColor.rgb - vec3(0.5)) * contrast + vec3(0.5);
float gray = dot(diffuseColor.rgb, vec3(0.299, 0.587, 0.114));
diffuseColor.rgb = mix(vec3(gray), diffuseColor.rgb, saturation);
`;

export const COLOR_FRAGMENT_WITH_HUE_GLSL = `
#include <color_fragment>
diffuseColor.rgb = (diffuseColor.rgb - vec3(0.5)) * contrast + vec3(0.5);
vec3 hsv = rgb2hsv(diffuseColor.rgb);
hsv.x = mod(hsv.x + hue / 360.0, 1.0);
diffuseColor.rgb = hsv2rgb(hsv);
float gray = dot(diffuseColor.rgb, vec3(0.299, 0.587, 0.114));
diffuseColor.rgb = mix(vec3(gray), diffuseColor.rgb, saturation);
`;
